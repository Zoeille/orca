...
export function CustomAgentDialog({
  ...
  const save = (): void => {
    ...
    if (!trimmedName || !trimmedCommand) {
      toast.error(
        translate(
          'auto.components.settings.CustomAgentDialog.required',
      ...
      <DialogContent className="sm:max-w-[560px]">
        <DialogHeader>
          <DialogTitle>
            {initialAgent
              ...
              : translate(
                  'auto.components.settings.CustomAgentDialog.createTitle',
                  'Create custom agent'
                )}
          </DialogTitle>
          <DialogDescription>
            {translate(
              ...
              'Configure a command Orca can launch in a terminal.'
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="custom-agent-name">
              {translate('auto.components.settings.CustomAgentDialog.name', 'Name')}
            </Label>
            <Input
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="custom-agent-command">
              {translate('auto.components.settings.CustomAgentDialog.command', 'Command')}
            </Label>
            <Input
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="custom-agent-prompt-mode">
              {translate(
                'auto.components.settings.CustomAgentDialog.promptMode',
                'Prompt delivery'
              )}
            </Label>
            <Select
              value={promptMode}
              onValueChange={(value) => setPromptMode(value as CustomAgentPromptMode)}
            >
              <SelectTrigger id="custom-agent-prompt-mode">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="pty">
                  {translate(
                    'auto.components.settings.CustomAgentDialog.pty',
                    'Type into the terminal'
                  )}
                </SelectItem>
                <SelectItem value="argv">
                  {translate(
                    'auto.components.settings.CustomAgentDialog.argv',
                    'Append as an argument'
                  )}
                </SelectItem>
                <SelectItem value="template">
                  {translate(
                    'auto.components.settings.CustomAgentDialog.template',
          ...
          {promptMode === 'template' && (
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="custom-agent-template">
                {translate(
                  'auto.components.settings.CustomAgentDialog.templateLabel',
                  'Command template'
                )}
              </Label>
              <Textarea
                id="custom-agent-template"
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  if (result) {
                    setIcon({ kind: 'image', dataUrl: result.dataUrl, fileName: result.fileName })
                  }
                } catch (error) {
                  toast.error(
                    error instanceof Error
                      ? error.message
                      : translate(
                          'auto.components.settings.CustomAgentDialog.importIconFailed',
            ...
            >
              ...
              {translate(
                'auto.components.settings.CustomAgentDialog.uploadIcon',
                'Choose PNG icon'
              )}
            </Button>
            <Button type="button" variant="ghost" size="sm" onClick={() => setIcon(defaultIcon)}>
              ...
              {translate(
                'auto.components.settings.CustomAgentDialog.terminalIcon',
                'Use terminal icon'
              )}
            </Button>
            {icon.kind === 'image' && (
              <img src={icon.dataUrl} alt="" aria-hidden className="size-7 rounded" />
            )}
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={save}>
            {translate('auto.components.settings.CustomAgentDialog.save', 'Save agent')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
