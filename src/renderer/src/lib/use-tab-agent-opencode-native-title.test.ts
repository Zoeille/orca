...
/** Mirrors the production narrowing: title resolution has no custom-agent vocabulary. */
function toTuiLaunchAgent(agent: AgentId | undefined): TuiAgent | undefined {
  return agent && isTuiAgent(agent) ? agent : undefined
}
  ...
  it('reclaims stale Claude identity loaded from a persisted tab', () => {
    ...
    expect(
      resolveTabAgentFromSignals({
        ...
        hookAgent: null,
        launchAgent: toTuiLaunchAgent(restoredTab.launchAgent)
      })
    ).toBe('opencode')
  })
