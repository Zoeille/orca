...
export function getTerminalQuickCommandAgentOptions(
  ...
  return [...catalog].sort((a, b) => {
    ...
    const aRank = isTuiAgent(a.id)
      ? (QUICK_COMMAND_AGENT_ORDER_RANK.get(a.id) ?? fallbackRank)
      : fallbackRank
    const bRank = isTuiAgent(b.id)
      ? (QUICK_COMMAND_AGENT_ORDER_RANK.get(b.id) ?? fallbackRank)
      : fallbackRank
    if (aRank !== bRank) {
      return aRank - bRank
    }
